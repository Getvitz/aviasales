...
const rootReducer = (state = defaultState, action) => {
    ...
    switch(action.type) {
        ...
        case "FILTER_ALL":
            if(state.filter.all) return defaultState;
            return {
                ...state,
                filter: {
                all: true,
        ...
        case "FILTER_NOTRANSFER":
            if(state.filter.notransfer) {return {
                ...state, 
        ...
        case "FILTER_ONETRANSFER":
            if(state.filter.onetransfer) {return {
                ...state, 
                filter: {...state.filter, onetransfer: false, all: false}
            }};
            return {
                ...state, filter: {...state.filter, onetransfer: true}
        }
        case "FILTER_TWOTRANSFER":
            if(state.filter.twotransfer) {return {
                ...state, 
                filter: {...state.filter, twotransfer: false, all: false}
            }};
            return {
                ...state, filter: {...state.filter, twotransfer: true}
            }
        case "FILTER_THREETRANSFER":
            if(state.filter.threetransfer) {return {
                ...state, 
                filter: {...state.filter, threetransfer: false, all: false}
            }};
            return {
                ...state, filter: {...state.filter, threetransfer: true}
        }
        case "SHOW_MORE":
            return {
